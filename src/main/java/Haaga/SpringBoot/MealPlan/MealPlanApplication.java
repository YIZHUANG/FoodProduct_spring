	...
	private static final Logger log = LoggerFactory.getLogger(MealPlanApplication.class);
		...
		public CommandLineRunner studentDemo(FoodRepository repository,MarketRepository mrepository,ProductTypeRepository trepository,UserRepository urepository) {
			return (args) -> {
				...
				repository.save(new FoodList("Minced lamb",4,"Finnish at its best",2000,"Hakaniemi","http://www.godfreys.co/ekmps/shops/fgodfrey/images/free-range-lean-lamb-mince-110-p.jpg",mrepository.findByMarketName("Kmarket").get(0),trepository.findByTypeName("Protein").get(0)));
